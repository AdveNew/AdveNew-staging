...
async function generateData() {
  ...
  for (let i = 1; i <= storeSeed; i += 1) {
    ...
    const calendar = [];
    const numberOfBookings = Math.floor((Math.random() * 101) + 200); // 200 - 300 bookings
    ...
    for (let j = 1; j <= numberOfBookings; j += 1) {
      ...
      const newBooking = {
        ...
        customerEmail: Faker.Random.element(eA),
    ...
    const store = {
      ...
      hours: 'Mon-Fri: '.concat(Faker.Number.between(7, 10)).concat('-').concat(Faker.Number.between(4, 6)).concat('\n     Sat-Sun: '.concat(Faker.Number.between(7, 10)).concat('-').concat(Faker.Number.between(1, 3))),
      calendar,
    };
